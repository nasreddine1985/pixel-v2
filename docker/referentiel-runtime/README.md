# Referentiel Runtime Docker

This directory contains the Docker configuration for building and running the referentiel service as a containerized Spring Boot application.

## Files

- `Dockerfile` - Multi-stage Docker build for the referentiel service
...
```bash
docker build -f referentiel-runtime/Dockerfile -t pixel-v2/referentiel-runtime:latest ../
```
...
```bash
# Run the referentiel service with dependencies
docker-compose up -d
...
# View logs
docker-compose logs -f referentiel-runtime
...
If you have an existing `docker-compose.yml` in the project root, you can include the referentiel service:

```bash
# From the project root
docker-compose -f docker/docker-compose.yml -f docker/referentiel-runtime/docker-compose.yml up -d
```
...
```bash
docker logs referentiel-service

# Follow logs
docker logs -f referentiel-service
```
...
```bash
docker ps
docker inspect referentiel-service
```
...
```bash
docker exec -it referentiel-service sh
```
