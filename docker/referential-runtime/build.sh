#!/bin/bash
# Build script for referential runtime Docker image
...
# Default values
IMAGE_NAME="pixel-v2/referential-runtime"
IMAGE_TAG="latest"
REGISTRY=""
PUSH_TO_REGISTRY=false
BUILD_CONTEXT="../"
DOCKERFILE_PATH="docker/referential-runtime/Dockerfile"
...
usage() {
    ...
    echo "  -n, --name NAME       Docker image name (default: pixel-v2/referential-runtime)"
...
echo -e "${BLUE}🐳 Building Docker image for referential runtime...${NC}"
...
if [[ ! -f "${DOCKERFILE_PATH}" && ! -f "referential-runtime/Dockerfile" ]]; then
...
if [[ -f "referential-runtime/Dockerfile" ]]; then
    BUILD_CONTEXT="../"
    DOCKERFILE_PATH="referential-runtime/Dockerfile"
fi
...
if [[ ! -d "${BUILD_CONTEXT}/referential" ]]; then
    echo -e "${RED}Error: referential directory not found in build context${NC}"
    echo -e "${YELLOW}Build context: ${BUILD_CONTEXT}${NC}"
    exit 1
fi
