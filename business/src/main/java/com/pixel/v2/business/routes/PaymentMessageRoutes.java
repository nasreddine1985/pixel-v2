package com.pixel.v2.business.routes;
...
import com.pixel.v2.business.processors.MessageTypeProcessor;
 ...
 * 
 * Route Flow:
 * 1. Listen for messages from Kafka (via k-kafka-message-receiver) OR direct HTTP calls from ingestion module
 * 2. Detect message type (pacs.008, pacs.009, pain.001, or camt.053)
...
public class PaymentMessageRoutes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        // Route for pacs.009 messages - calls k-pacs009-to-cdm-transformer kamelet
        from("direct:pacs-009-transform")
            .routeId("pacs-009-transformation")
            .log(LoggingLevel.INFO, "[PACS-009-TRANSFORM] Processing pacs.009 message")
            .to("kamelet:k-pacs009-to-cdm-transformer")
            .log(LoggingLevel.INFO, "[PACS-009-TRANSFORM] Successfully transformed pacs.009 to CDM")
            .to(cdmOutputEndpoint);

        // Route for pain.001 messages - calls k-pain001-to-cdm-transformer kamelet
        from("direct:pain-001-transform")
            .routeId("pain-001-transformation")
            .log(LoggingLevel.INFO, "[PAIN-001-TRANSFORM] Processing pain.001 message")
            .to("kamelet:k-pain001-to-cdm-transformer")
            .log(LoggingLevel.INFO, "[PAIN-001-TRANSFORM] Successfully transformed pain.001 to CDM")
            .to(cdmOutputEndpoint);

        // Route for camt.053 messages - calls k-camt053-to-cdm-transformer kamelet
        from("direct:camt-053-transform")
            .routeId("camt-053-transformation")
            .log(LoggingLevel.INFO, "[CAMT-053-TRANSFORM] Processing camt.053 message")
            .to("kamelet:k-camt053-to-cdm-transformer")
            .log(LoggingLevel.INFO, "[CAMT-053-TRANSFORM] Successfully transformed camt.053 to CDM")
            .to(cdmOutputEndpoint);
        ...
        // Direct HTTP endpoints for ingestion module integration
        from("platform-http:/business/api/direct/pacs-008-transform")
            .routeId("http-pacs-008-direct")
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Received PACS.008 message from ingestion module")
            .setHeader("MessageType", constant("pacs.008"))
            .setHeader("RouteTarget", constant("direct:pacs-008-transform"))
            .to("direct:pacs-008-transform");

        from("platform-http:/business/api/direct/pacs-009-transform")
            .routeId("http-pacs-009-direct")
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Received PACS.009 message from ingestion module")
            .setHeader("MessageType", constant("pacs.009"))
            .setHeader("RouteTarget", constant("direct:pacs-009-transform"))
            .to("direct:pacs-009-transform");

        from("platform-http:/business/api/direct/pain-001-transform")
            .routeId("http-pain-001-direct")
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Received PAIN.001 message from ingestion module")
            .setHeader("MessageType", constant("pain.001"))
            .setHeader("RouteTarget", constant("direct:pain-001-transform"))
            .to("direct:pain-001-transform");

        from("platform-http:/business/api/direct/camt-053-transform")
            .routeId("http-camt-053-direct")
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Received CAMT.053 message from ingestion module")
            .setHeader("MessageType", constant("camt.053"))
            .setHeader("RouteTarget", constant("direct:camt-053-transform"))
            .to("direct:camt-053-transform");

        from("platform-http:/business/api/direct/kafka-message-processing")
            .routeId("http-general-processing-direct")
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Received message from ingestion module for general processing")
            .process(messageTypeProcessor)
            .log(LoggingLevel.INFO, "[HTTP-DIRECT] Message type detected: ${header.MessageType}, routing to: ${header.RouteTarget}")
            .recipientList(simple("${header.RouteTarget}"))
            .end();
