...
# Check Artemis MQ
if [ -n "$PIXEL_MQ_URL" ]; then
    echo "Waiting for MQ to be ready..."
    echo "MQ URL: $PIXEL_MQ_URL"
    MQ_HOST=$(echo $PIXEL_MQ_URL | sed 's/.*:\/\/\([^:]*\).*/\1/')
    MQ_PORT=$(echo $PIXEL_MQ_URL | sed 's/.*:\([0-9]*\).*/\1/')
    echo "Parsed MQ Host: $MQ_HOST, Port: $MQ_PORT"
    until nc -z "$MQ_HOST" "$MQ_PORT" 2>/dev/null; do
        echo "MQ not ready ($MQ_HOST:$MQ_PORT), waiting 5 seconds..."
        sleep 5
    done
    echo "MQ is ready!"
fi
...
exec jbang \
    ...
    -R="-Djava.rmi.server.hostname=pixel-v2-app" \
