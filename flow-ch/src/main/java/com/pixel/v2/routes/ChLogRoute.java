...
public class ChLogRoute extends RouteBuilder {
        ...
        public void configure() throws Exception {
                ...
                from("kamelet:k-kafka-starter?bootstrapServers={{ch.log.kafka.brokers}}&topic={{kmq.starter.kafkaLogTopicName}}&groupId={{ch.log.kafka.groupId}}&offsetReset={{ch.log.kafka.offsetReset}}&sinkEndpoint=direct:process-log-events")
                                .routeId("ch-log-events-processing-flow")
                ...
                from("direct:process-flow-summary-log").routeId("ch-log-main-processing")
                                // .log("Received flow summary message from k-kafka-starter:
                                // ${body}")
                ...
                from("direct:persist-flow-summary-message").routeId("ch-log-message-persistence")
                                .log("Persisting flow summary message with Kafka metadata - Topic: ${header.kafkaTopic}, Offset: ${header.kafkaOffset}")
                                .setHeader("RouteName", constant("CH-Log-Processing"))
                                .setHeader("ProcessingNode", simple("${sys.HOSTNAME}"))
                                // Log the message details
                                // .log("📊 CH-LOG: Flow Summary Data Received - Message Body
                                // Length:
                                // ${body.length()}")
                                // .log("📊 CH-LOG: Kafka Metadata - Topic: ${header.kafkaTopic},
                                // Partition:
                ...
                // Processing route: Handle log events message and persist to database
                from("direct:process-log-events").routeId("ch-log-events-processing")
                                // .log("Received log events message from k-kafka-starter: ${body}")
                ...
                from("direct:persist-log-events-message").routeId("ch-log-events-persistence").log(
                                ...
                                .setHeader("ProcessingNode", simple("${sys.HOSTNAME}"))
                                // Log the message details
                                // .log("📋 CH-LOG: Log Events Data Received - Message Body Length:
                                // ${body.length()}")
                                // .log("📋 CH-LOG: Kafka Metadata - Topic: ${header.kafkaTopic},
                                // Partition:
                                // ${header.kafkaPartition}, Offset: ${header.kafkaOffset}")
                                // .log("📋 CH-LOG: Message Content: ${body}")
                                // Use the new k-db-log-events kamelet for JPA persistence
                                .to("kamelet:k-db-log-events")
                // .log("✅ CH-LOG: Log events message persisted successfully to database via
                // k-db-log-events kamelet")
                ;
