 ...
 * 2. Database persistence (initial message save)
 * 3. Reference data enrichment
 * 4. Enriched data persistence (save enriched data)
 * 5. Validation
 * 6. Idempotence checking
 * 7. Publishing to Kafka
 */
@Component
@Profile("!test")
    ...
    public void configure() throws Exception {
        ...
        if (activeProfiles == null || !activeProfiles.contains("test")) {
            from("kamelet:k-mq-message-receiver?queueName={{ingestion.mq.input.queue:PAYMENT_INPUT}}&host={{ingestion.mq.host:localhost}}&port={{ingestion.mq.port:1414}}&queueManager={{ingestion.mq.queue.manager:QM1}}&channel={{ingestion.mq.channel:DEV.ADMIN.SVRCONN}}&username={{ingestion.mq.username:admin}}&password={{ingestion.mq.password:admin}}")
                ...
                .setHeader("ReceiptChannel", constant("MQ"))
        ...
        from("direct:database-persistence")
            ...
            .log("Persisting message to database via k-database-transaction")
        ...
        // Processing publisher step - for HTTP/MQ messages
