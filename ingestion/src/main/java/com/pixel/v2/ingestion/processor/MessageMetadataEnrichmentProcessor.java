...
/**
 * Processor to enrich messages with comprehensive metadata for processing module integration. This
 * processor creates a standardized message format with metadata that supports both Kafka and direct
 * processing routing paths.
 * 
 * Supported message types: - pacs.008: ISO 20022 Customer Credit Transfer Initiation - pacs.009:
 * ISO 20022 Financial Institution Credit Transfer Return - pain.001: ISO 20022 Customer Payment
 * Initiation - camt.053: ISO 20022 Bank-to-Customer Account Statement
 */
@Component
public class MessageMetadataEnrichmentProcessor implements Processor {

    private static final Logger log =
            LoggerFactory.getLogger(MessageMetadataEnrichmentProcessor.class);
    ...
    public void process(Exchange exchange) throws Exception {
        log.debug(
                "[METADATA-ENRICHMENT] Starting message metadata enrichment for processing module");
        ...
        try {
            ...
            log.info(
                    "[METADATA-ENRICHMENT] Message enriched with metadata for processing module. "
                            + "Route: {} → Processing Module",
                    exchange.getIn().getHeader(RECEIPT_CHANNEL_HEADER));

        } catch (Exception e) {
            log.error("[METADATA-ENRICHMENT] Failed to enrich message metadata: {}", e.getMessage(),
                    e);
    ...
    private ObjectNode createMetadataSection(Exchange exchange) {
        ...
        metadata.put("ingestionStartTime",
                getHeaderAsString(exchange, INGESTION_START_TIME_HEADER));
        ...
        metadata.put("enrichedDataPersisted",
                getHeaderAsBoolean(exchange, ENRICHED_DATA_PERSISTED_HEADER));

        // Persistence IDs if available
        if (exchange.getIn().getHeader(PERSISTED_MESSAGE_ID_HEADER) != null) {
            metadata.put("persistedMessageId",
                    getHeaderAsString(exchange, PERSISTED_MESSAGE_ID_HEADER));
        }
