 ...
 * - XslFileName: Name of the XSL file (required)
 * - TransformationMode: STRICT (fail on error) or LENIENT (continue on error)
 * 
 * Output headers:
 * - TransformationStatus: SUCCESS, ERROR, or PARTIAL (for collections with some failures)
 * - TransformationScope: SINGLE or COLLECTION
 * - TransformationCount: Number of messages processed
 * - TransformationSuccessCount: Number of successful transformations
 * - TransformationErrorCount: Number of failed transformations
 * - TransformationDuration: Processing duration in milliseconds
 * - TransformationTimestamp: ISO timestamp of completion
...
public class XslTransformationProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        try {
            ...
            String transformationMode = exchange.getIn().getHeader("TransformationMode", "STRICT", String.class);
            ...
            if (body instanceof Collection) {
                processCollection(exchange, templates, (Collection<?>) body, transformationMode, startTime);
            } else if (body instanceof String) {
                processSingleMessage(exchange, templates, (String) body, startTime);
            } else {
                throw new XslTransformationException("Unsupported body type: " + body.getClass().getName() + 
                    ". Expected String or Collection<String>");
            }
    ...
    private void processCollection(Exchange exchange, Templates templates, Collection<?> messages, 
        ...
        for (Object message : messages) {
            try {
                if (!(message instanceof String)) {
                    throw new XslTransformationException("Message #" + messageIndex + " is not a String: " + 
                ...
                logger.debug("[XSL-TRANSFORMATION] Message #{} transformed successfully", messageIndex);
                
            } catch (Exception e) {
                errorCount++;
                String errorMsg = "Message #" + messageIndex + " transformation failed: " + e.getMessage();
                errors.add(errorMsg);
        ...
        setHeaders(exchange, status, "COLLECTION", messages.size(), successCount, errorCount, errorDetails, startTime);
        
        if ("ERROR".equals(status) && "STRICT".equals(transformationMode)) {
            throw new XslTransformationException(errorDetails);
        }
        
        logger.info("[XSL-TRANSFORMATION] ✅ Collection transformation completed - Success: {}, Errors: {}, Status: {}", 
     ...
     */
    private String transformXml(String xmlContent, Templates templates) throws TransformerException {
    ...
    private Templates loadXslTemplate(String xslFileName) throws XslTransformationException {
        try {
            ...
            try (InputStream xslStream = getClass().getResourceAsStream(xslPath)) {
                if (xslStream == null) {
                    throw new XslTransformationException("XSL stylesheet file not found: " + xslFileName + 
                        " (looking in classpath: " + xslPath + ")");
                }
            ...
            throw new XslTransformationException("Failed to parse XSL stylesheet '" + xslFileName + "': " + 
                e.getMessage(), e);
        } catch (Exception e) {
            throw new XslTransformationException("Failed to load XSL stylesheet '" + xslFileName + "': " + 
                e.getMessage(), e);
        }
    }
    ...
    private void handleError(Exchange exchange, Exception e, long startTime) {
        logger.error("[XSL-TRANSFORMATION] ❌ Transformation failed - XSL: {}, Duration: {}ms, Error: {}", 
                    exchange.getIn().getHeader("XslFileName"), 
        ...
        exchange.getIn().setHeader("TransformationDuration", System.currentTimeMillis() - startTime);
        exchange.getIn().setHeader("TransformationTimestamp", java.time.Instant.now().toString());
    }
}