...
# Check if we're in the correct directory - handle both root and scripts directory
...
if [ -f "docker/${COMPOSE_FILE}" ]; then
    ...
    PROJECT_ROOT="."
elif [ -f "../docker/${COMPOSE_FILE}" ]; then
    # We're in the scripts directory, go up one level
    PROJECT_ROOT=".."
    cd ..
else
    print_error "docker-compose.yml not found. Please run this script from the PIXEL-V2 root directory or scripts directory."
    exit 1
fi
