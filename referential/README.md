...
The Referential Configuration Service is a Spring Boot REST API that provides configuration management for the PIXEL-V2 payment message processing system. It manages flow configurations, country mappings, partner settings, business rules, and character encodings.
...
```bash
# From project root
./start-referential.sh                    # Build and start service
./start-referential.sh --build-only      # Build only
./start-referential.sh --rebuild --logs  # Rebuild and follow logs
./start-referential.sh --stop            # Stop service
./start-referential.sh --restart         # Restart service
```
...
3. **Container Issues**
   ...
   docker logs pixel-v2-referential

   # Check container status
   docker ps --filter name=pixel-v2-referential
   ```
...
```properties
logging.level.com.pixel.v2.referential=DEBUG
