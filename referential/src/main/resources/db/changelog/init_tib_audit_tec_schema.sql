...
-- MQ Series Transport Configuration
CREATE TABLE tib_audit_tec.ref_transport_kafka (
    transport_id BIGINT PRIMARY KEY,
    kafka_topic_name VARCHAR(50) NOT NULL,
    kafka_group_id VARCHAR(30) NOT NULL,
    FOREIGN KEY (transport_id) REFERENCES tib_audit_tec.ref_transport(transport_id),
    UNIQUE (kafka_topic_name, kafka_group_id)
);
...
CREATE TABLE tib_audit_tec.ref_flow_rules (
    ...
    CONSTRAINT ref_flow_rules_pkey PRIMARY KEY (flowcode, transporttype),
...
COMMENT ON TABLE tib_audit_tec.ref_transport_kafka IS 'Apache Kafka transport-specific configuration';
